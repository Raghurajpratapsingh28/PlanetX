...
export default function DashboardPage() {
  ...
  const filteredProperties = properties
    .filter(property => 
      searchId ? property.id.includes(searchId) : true
    )
    .filter(property => 
      filterStatus === "All" ? true : property.status === filterStatus
    )
    .sort((a, b) => {
      if (sortBy === "Latest") return new Date(b.addedDate) - new Date(a.addedDate);
      if (sortBy === "Oldest") return new Date(a.addedDate) - new Date(b.addedDate);
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          { title: "Active Property", value: properties.filter(p => p.status === "Active").length, icon: "✅" },
          { title: "On-Hold Property", value: properties.filter(p => p.status === "On-Hold").length, icon: "⏳" },
        ].map((stat, index) => (
          <motion.div
            key={index}
            initial={{ opacity: 0, y: 20 }}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2 sm:mt-3">
                  ...
                  <p className="text-xs sm:text-sm text-gray-500">Added {property.addedDate || "N/A"}</p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <select
